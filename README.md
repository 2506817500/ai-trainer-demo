# ai-trainer-demo
AI训练师入门练习项目，Python实现SFT数据集处理、RLHF偏好样本构建、数据内容审核与数据集质量质检。

## 项目简介
本项目模拟AI训练师完整工作流：SFT指令标注、RLHF偏好样本构建、文本清洗、脏数据过滤、数据集内容审核、数据集自动化质检，输出标准jsonl格式训练样本。
...
6. 数据集自动化质检：自动识别JSON格式错误、空样本、非法字符，输出质检统计报告
...
- data_quality_check.py：数据集质量自动质检脚本，输出质检报告，识别脏样本
- test.py：代码调试测试文件
